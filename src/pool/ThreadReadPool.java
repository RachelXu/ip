...
public class ThreadReadPool extends ThreadGroup {
	private boolean isClosed = false; // 线程池是否关闭
	private LinkedList workQueue; // 工作队列
	private static int threadPoolID = 2; // 线程池的id

	public ThreadReadPool(int poolSize) { // poolSize 表示线程池中的工作线程的数量
	...
	private synchronized Runnable getTask(int threadid)
		...
		while (workQueue.size() == 0) {
			if (isClosed)
				return null;
			System.out.println("读工作线程" + threadid + "等待任务...");
			wait(); // 如果工作队列中没有任务,就等待任务
		}
		System.out.println("读工作线程" + threadid + "开始执行读任务...");
